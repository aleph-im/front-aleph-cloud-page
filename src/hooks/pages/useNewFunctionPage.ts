...
export type NewFunctionPage = {
  ...
  setEnvironmentVariable: (variableIndex: number, variableKey: "name" | "value", variableValue: string) => void
  addEnvironmentVariable: () => void
  removeEnvironmentVariable: (variableIndex: number) => void
  setVolumeType: (volumeIndex: number, volumeType: number) => void
  setVolumeValue: (volumeIndex: number, volumekey: keyof Volume, volumeValue: any) => void
  addVolume: () => void
...
export function useNewFunctionPage(): NewFunctionPage {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const alephVolumes = await displayVolumesToAlephVolumes(account, formState.volumes)
