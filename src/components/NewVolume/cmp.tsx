...
import { Tabs, Icon, TextInput, Checkbox, Button } from "@aleph-front/aleph-core";
...
const RemoveVolume = ({ removeCallback }: RemoveVolumeProps) => (
  <div className="my-md text-right">
    <Button type="button" onClick={removeCallback} color="main2" variant="secondary" kind="neon" size="regular">
      Remove
    </Button>
  </div>
)
...
export default function NewVolume({ volumeMountpoint, volumeRefHash, volumeName, volumeSize, volumeSrc, volumeUseLatest, handleMountpointChange, handleNameChange, handleSizeChange, handleSrcChange, handleUseLatestChange, handleRefHashChange, handleVolumeType, removeCallback, isStandAlone }: NewVolumeProps){
  const namePrefix = useMemo(() => (Math.abs(Math.random() * 10**16 | 0).toString(16)), [])
            ...
            <NoisyContainer>
              ...
              <div className="my-md">
                <TextInput
                  ...
                  value={volumeRefHash}
                  error={
                    (volumeRefHash && !isValidItemHash(volumeRefHash || '')) ? { message: "Invalid hash" } : undefined
                  }
            ...
            <NoisyContainer>
              <div className="my-md">
                <TextInput
                  ...
                  error={volumeName === "" ? new Error('Please provide a name') : undefined} />
              </div>
