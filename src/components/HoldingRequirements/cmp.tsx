import { ellipseAddress, humanReadableCurrency, getFunctionCost, convertBitUnits } from "@/helpers/utils";
...
export default function HoldingSummary({address, computeUnits, storage, unlockedAmount}: HoldingRequirementsProps) {
  ...
  const getVolumeSize = (volume: VolumeRequirements) => {
    if(volume.type === 'new')
    return convertBitUnits(volume?.src?.size || 0, { from: 'b', to: 'gb', displayUnit: false}) as number
    return volume.size
  }

  const displayedVolumes: (VolumeRequirements & { price: number })[] = useMemo(() => {
    const onlyNewVolumes = storage?.filter((volume) => volume.type !== 'existing') || []
    if(!computeUnits){
  ...
  const totalCost = useMemo(() => {
    const volumeCost = displayedVolumes.reduce((acc, volume) => acc + volume.price, 0)
    ...
    <>
...

      {
        ...
        <StyledHoldingSummaryLine>
          ...
          <div>{computeUnits.number} x86 64bit {computeUnits.isPersistent && '(persistent)'}</div>
      ...
      <StyledHoldingSummaryLine>
        ...
        <div>
          <TextGradient type="body2" color="main1">{humanReadableCurrency(totalCost)} ALEPH</TextGradient> 
        </div>
      </StyledHoldingSummaryLine>
    </>
  )
