...
export type UseNewDomainFormReturn = {
...
export function useNewDomainForm({
  ...
  const onSubmit = useCallback(
    ...
    [dispatch, manager, next, onSuccess, stop],
  )
  ...
  const currentEntityType = targetCtrl.field.value

  const entities = useMemo(() => {
    if (!currentEntityType) return []
    if (currentEntityType !== EntityDomainType.IPFS) {
      const entities =
        currentEntityType === EntityDomainType.Instance
          ? instances
          : currentEntityType === EntityDomainType.Confidential
            ? confidentials
            : programs
  ...
  }, [currentEntityType, instances, programs, websites, confidentials])
  ...
  useEffect(() => {
    if (currentEntityType === EntityDomainType.Instance && hasInstances) {
    ...
    } else if (currentEntityType === EntityDomainType.Program && hasFunctions) {
      setValue('target', EntityDomainType.Program)
    } else if (currentEntityType === EntityDomainType.IPFS) {
